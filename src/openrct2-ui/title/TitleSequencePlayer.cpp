        ...
        bool Update() override
        {
            ...
            while (true)
            {
                ...
                catch (std::exception& e)
                {
                    ...
                    {
                        // OPENRCT2MINI: a partially-imported park leaves gameState half-mutated
                        // (e.g. some objects loaded, ride table half-populated when a ride id
                        // overshot kMaxRidesInPark). Rendering that state produces visibly corrupt
                        // terrain *and* corrupt entity/ride behaviour. Per project policy: any
                        // load failure abandons the entire sequence so the title scene falls back
                        // to a different one. We reset the world to a clean default first so the
                        // briefly-corrupt state cannot be drawn between Update returning false and
                        // the next sequence's first park loading.
                        Console::Error::WriteLine("  Abandoning title sequence and falling back to another.");
