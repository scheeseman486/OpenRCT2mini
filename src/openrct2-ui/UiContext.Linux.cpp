...
namespace OpenRCT2::Ui
{
    ...
    public:
        ...
        {
            // OPENRCT2MINI cut 53: hardwire to false. The native file-picker
            // path shells out to zenity / kdialog via Platform::Execute,
            // which on the Miyoo Mini doesn't exist (the OnionUI rootfs has
            // neither), and on dev hosts pops up a desktop dialog that's
            // useless during fullscreen 640×480 cursor-driven gameplay.
            // The in-game LoadSave window (Windows::LoadsaveOpen) is the
            // intended UX for both targets — returning false here forces
            // OpenPreferred() to take that path regardless of what
            // use_native_browse_dialog is set to in config.ini.
            return false;
        }
