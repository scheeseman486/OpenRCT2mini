...
#pragma region Vehicle
        ...
        void VehicleOnScrollDraw(RenderTarget& rt, int32_t scrollIndex)
        {
            ...
            for (int32_t i = 0; i < ride->numTrains; i++)
            {
                ...
                // OPENRCT2MINI: with kMaxCarsPerTrain reduced to RCT2 original (32), clamp at runtime
                // since numCarsPerTrain's type is uint8_t (max 255).
                int32_t numCars = std::min<int32_t>(ride->numCarsPerTrain, static_cast<int32_t>(std::size(trainCarImages)));
