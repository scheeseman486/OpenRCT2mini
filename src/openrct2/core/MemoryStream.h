...
namespace OpenRCT2
{
    ...
    public:
        // OPENRCT2MINI: pre-reserve write capacity. Hint the stream that we're about to
        // write `length` bytes so it allocates once instead of doubling through many
        // intermediate sizes. Cuts the dominant realloc churn during chunk decoding
        // (DecodeChunkRLE / Repeat / Rotate write byte-by-byte over the entire output).
