...
#pragma pack(pop)
    ...
    // OPENRCT2MINI: data ownership uses a function-pointer deleter so the same
    // pointer type can hold either heap-allocated (delete[]) or mmap'd (munmap)
    // sprite data. See SpritePager.h for helpers. Default: empty no-op deleter.
    struct Gx
    {
        G1Header header;
        std::vector<G1Element> elements;
        std::unique_ptr<uint8_t[], void (*)(uint8_t*)> data{ nullptr, +[](uint8_t*) noexcept {} };
    };
