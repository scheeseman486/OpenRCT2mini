...
std::optional<Gx> GfxLoadGx(const std::vector<uint8_t>& buffer)
{
    try
    {
        ...
        // Read element data
        gx.data = OpenRCT2::Drawing::MakeHeapSpriteData(istream.ReadArray<uint8_t>(gx.header.totalSize));
...
static std::optional<PaletteMap> FASTCALL GfxDrawSpriteGetPalette(ImageId imageId)
{
    ...
    // spell out span<PaletteIndex> explicitly so the shim works in both modes.
    auto paletteMap = PaletteMap(std::span<PaletteIndex>(reinterpret_cast<PaletteIndex*>(kSecondaryRemapPalette), 256));
    if (imageId.HasTertiary())
    {
        paletteMap = PaletteMap(std::span<PaletteIndex>(reinterpret_cast<PaletteIndex*>(kTertiaryRemapPalette), 256));
