...
// OPENRCT2MINI: cut 33. <ranges> dropped for C++17 compat — std::lower_bound
// with an explicit comparator replaces the ranges projection-based form.
#include <algorithm>
#include <utility>
...
static void putExtraImpl(IntentDataStorage& storage, uint32_t key, T&& value)
{
    auto it = std::lower_bound(
        storage.begin(), storage.end(), key,
        [](const IntentDataEntry& entry, uint32_t k) { return entry.first < k; });
