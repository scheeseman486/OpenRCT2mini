...
namespace OpenRCT2::TrackMetadata
{
    ...
    // OPENRCT2MINI: cut 33. Originally a union with constexpr active-member
    // switching — that became valid in C++20 (P1330R0) but is ill-formed in
    // C++17 constant evaluation. Replaced with two plain members + a tag;
    // costs an extra few bytes per instance but compiles cleanly in both
    // standards and behaves identically at runtime.
    struct TypeOrCurve
    {
        bool isTrackType = true;
        OpenRCT2::TrackElemType trackType = OpenRCT2::TrackElemType::none;
        TrackCurve curve = TrackCurve::none;
        ...
        constexpr bool operator!=(OpenRCT2::TrackElemType rhs) const { return !(*this == rhs); }
        constexpr bool operator!=(TrackCurve rhs) const { return !(*this == rhs); }

        constexpr TypeOrCurve(OpenRCT2::TrackElemType _type) noexcept
            : isTrackType(true), trackType(_type)
        {
        }
        ...
        constexpr TypeOrCurve(TrackCurve _curve) noexcept
            : isTrackType(false), curve(_curve)
        {
        }
