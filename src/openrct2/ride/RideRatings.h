...
#pragma pack(push, 1)
        ...
        struct Tuple
        {
            ...
            // OPENRCT2MINI: cut 33. Defaulted operator== is C++20; spell it out
            // for C++17 compat. Same semantics — member-wise equality.
            bool operator==(const Tuple& rhs) const noexcept
            {
                return excitement == rhs.excitement && intensity == rhs.intensity && nausea == rhs.nausea;
            }
            bool operator!=(const Tuple& rhs) const noexcept { return !(*this == rhs); }
        };
