...
namespace OpenRCT2::RideVehicle::Geometry
{
    ...
    // clang-format off
    /** rct2: 0x009A2930 */
    // OPENRCT2MINI: cut 36. Add explicit <int> so the C++17 polyfill matches.
    constexpr auto kSubpositionTranslationDistances = std::array<int, 16>{{
    ...
    /** rct2: 0x009A36C4 */
    const auto kFreeroamVehicleMovementData = std::array<Unk9A36C4Struct, 32>{{
    ...
    /** rct2: 0x009A2970 */
    constexpr auto kAccelerationFromPitch = std::array<int32_t, 61>{{
        0,       // Flat
     ...
     *  rct2: 0x009A38D4
     */
    constexpr auto kPitchToDirectionVectorInt32 = std::array<CoordsXY, 61>{{
        { 2147483647, 0 },            // flat
    ...
    /** rct2: 0x009A39C4 */
    const auto kRollHorizontalComponent = std::array<int32_t, 20>{{
    ...
    /** rct2: 0x009A3684 */
    const auto kSpriteDirectionToSoundDirection = std::array<int32_t, 32>{{
        -0x4000, // 0
        -0x3000, // 1
        -0x2000, // 2
        -0x1000, // 3
    ...
    const auto kCrashDirectionComponents = std::array<CoordsXY, 16>{{
        { -256, 0 },
        { -236, 98 },
        { -181, 181 },
        { -98, 236 },
