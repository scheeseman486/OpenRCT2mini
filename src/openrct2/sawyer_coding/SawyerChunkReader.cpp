...
namespace OpenRCT2::SawyerCoding
{
    ...
    {
        MemoryStream buf;
        // OPENRCT2MINI: pre-reserve a *modest* hint to avoid the worst of the realloc-
        // double-double sequence without over-committing for huge chunks. RLE realistic
        // expansion is 2–4x; we reserve 2x to keep the typical case alloc-once but cap
        // at 2 MiB so a multi-MB compressed chunk doesn't peak at 16 MB upfront. If the
        // decoded size exceeds the reservation, EnsureWriteCapacity still grows correctly.
        {
            constexpr uint64_t kReserveCap = 2 * 1024 * 1024;
            uint64_t hint = static_cast<uint64_t>(srcLength) * 2;
            if (hint > kReserveCap) hint = kReserveCap;
            buf.Reserve(hint);
        }
    ...
    static MemoryStream DecodeChunkRepeat(const void* src, size_t srcLength)
    {
        MemoryStream buf;
        {
            // OPENRCT2MINI: see DecodeChunkRLE — modest pre-reserve to avoid realloc churn
            // without over-committing for big chunks.
            constexpr uint64_t kReserveCap = 2 * 1024 * 1024;
            uint64_t hint = static_cast<uint64_t>(srcLength) * 2;
            if (hint > kReserveCap) hint = kReserveCap;
            buf.Reserve(hint);
    ...
    {
        MemoryStream buf;
        // OPENRCT2MINI: rotate is 1:1, so reserve exact output size up front.
