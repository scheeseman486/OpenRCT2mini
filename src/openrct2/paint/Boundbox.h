            ...
            boundBoxes)
{
    ...
    for (auto& view : flippedBoundBoxes)
    {
        for (auto& trackSequence : view)
        {
            ...
            {
                // OPENRCT2MINI: cut 33. std::swap is constexpr only in C++20;
                // hand-rolled three-step swap works in any constexpr context.
                {
                    auto t = boundBox.offset.x; boundBox.offset.x = boundBox.offset.y; boundBox.offset.y = t;
                }
                {
                    auto t = boundBox.length.x; boundBox.length.x = boundBox.length.y; boundBox.length.y = t;
                }
            }
        }
    }
