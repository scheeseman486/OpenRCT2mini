...
namespace OpenRCT2
{
    ...
    public:
        ...
        {
            // OPENRCT2MINI: cut 33. Heterogeneous unordered_map lookup
            // (find(string_view) on map<string,...>) is C++20 P0919.
            // C++17 requires key-type match — explicit string construction.
            ...
            std::string_view path, const RCTObjectEntry* entry, const void* data, size_t dataSize, bool fixChecksum = true)
        {
            if (fixChecksum)
            {
                ...
                if (realChecksum != entry->checksum)
                {
                    ...
                    const auto dataSpan = std::span<const uint8_t>(static_cast<const uint8_t*>(data), dataSize);
            ...
            // OPENRCT2MINI: cut 33. std::make_unique_for_overwrite is C++20.
            // The C++17 way to allocate without value-init is `new T[N]` raw.
            const auto encodedDataBuffer = std::unique_ptr<uint8_t[]>(new uint8_t[0x600000]);
