...
namespace OpenRCT2
{
    ...
    {
        // OPENRCT2MINI: keep only the current language plus an English-UK fallback for
        // missing translations. Every Object normally stores ~3 strings × 8+ languages —
        // most of those are read at startup, allocate std::string heap storage if the
        // text overflows SSO, and are then never queried at runtime. Filtering at read
        // time saves both the std::string instances and the arena fragmentation those
        // many small allocations cause. GetString() already falls back to English-UK
        // when the requested language isn't found, so the user-visible behaviour matches.
        const auto& localisationService = GetContext()->GetLocalisationService();
        const auto currentLanguageId = static_cast<uint8_t>(localisationService.GetCurrentLanguage());
        try
        {
            ...
            while ((rct2LanguageId = static_cast<RCT2LanguageId>(stream->ReadValue<uint8_t>())) != RCT2LanguageId::end)
            {
                ...
                // Skip the conversion and store entirely if this isn't a language we need.
                const bool wantThisLanguage = (languageId == currentLanguageId)
                    || (languageId == LANGUAGE_ENGLISH_UK);
                if (!wantThisLanguage)
                {
                    continue;
                }
        ...
        Sort();
        // OPENRCT2MINI: vector::push_back doubles capacity, so a StringTable that ended up
        // with 5 entries may have capacity 8. Trim to the exact size — the table is read
        // once at Object load and never grown afterward.
